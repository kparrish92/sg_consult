...
An Bayesian Ordinal regression fit in R with `brms` with default priors with the cumulative family. Rating was predicted as a function of survey version (1 or 2) and question type (4 levels), with random intercepts for prompt and participant. The model was fit with 1000 warm-up and 2000 additional iterations accross 4 processing cores. 
...
Figure 1 shows the distribution of ratings in each of the 4 question types in both versions 1 and 2. 
...
The general trends show a right skewed distribution for the most important questions and semi-affected questions, a left skew for directly affected questions, and a normal distribution for least affected questions. 
Based on this plot alone, there seem to be slightly higher ratings for verson 1 than 2 in all of the question types expect for directly affected questions.
...
The model, though, does not give the areas of the rectangle, but rather the exact location in which we would draw a vertical line to make one rectangle two conjoined ones. The long side of the rectangle is generally ranges from 0-1 in probability and the estimates tell us where (in log-odds), we can "draw" the first cut point. For example, `Intercept [1]` is the first cutoff point and represents the log-odds of a choice of 1 in the baseline condition (directly affected questions in version 1). The effect is `r report_post$Median[1]` (equal to a probability of `r round(plogis(report_post$Median[1]), digits = 2)`). We can interpret this as strong evidence that 1 is the most probable response in the baseline condition, and it suggests that the cutoff point could range from  `r report_post$CI_low[1]` (probability = `r round(plogis(report_post$CI_low[1]),digits = 2)`) to `r report_post$CI_high[1]` (probability `r round(plogis(report_post$CI_high[1]), digits = 2)`).

The fixed effect `b_version2` tells us the overall shift in ratings going from version 1 to version 2 in the baseline. 
In this case, we do not have evidence of a compelling difference. 
We say this because, first, the highest density interval (HDI) overlaps zero [95% HDI `r report_post$Median[5]`, `r report_post$CI_low[5]` - `r report_post$CI_high[5]`], and the probability of direction `r report_post$pd[5]`. 
On the other hand, `b_current_class` did have a small impact on ratings, in which a one unit increase in class was associated with a change in `r report_post$Median[9]` log-odds [95% HDI `r report_post$CI_low[9]` - `r report_post$CI_high[9]`]. 
...
The research questions (specifically question 2) are best answered by the interaction terms `b_version2:question_typeleast_affected_questions`, `b_version2:question_typemost_interested_qs` and `b_version2:question_typesemi_affected_questions`, since these estimates test the differences between versions for each question type. 
In this case, the model predicted with compelling evidence that version 2 was rated lower for the most important questions 
[`r report_post$Median[11]`; 95% HDI `r report_post$CI_low[11]` - `r report_post$CI_high[11]`], semi-affected questions 
[`r report_post$Median[12]`; 95% HDI `r report_post$CI_low[12]` - `r report_post$CI_high[12]`] and the least affected questions 
[`r report_post$Median[10]`; 95% HDI `r report_post$CI_low[10]` - `r report_post$CI_high[10]`].
