...
On the other hand, `b_current_class` did have a small impact on ratings, in which a one unit increase in class was associated with a change in `r report_post$Median[8]` log-odds [95% HDI `r report_post$CI_low[8]` - `r report_post$CI_high[8]`]. 
...
The research questions (specifically question 2) are best answered by the interaction terms `b_version2:question_typeleast_affected_questions`, and `b_version2:question_typesemi_affected_questions`, since these estimates test the differences between versions for each question type. 
In this case, the model did not predict with compelling evidence that version 2 was rated lower for the semi-affected questions 
[`r report_post$Median[10]`; 95% HDI `r report_post$CI_low[10]` - `r report_post$CI_high[10]`] and the least affected questions 
